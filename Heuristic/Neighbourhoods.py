...
def calculate_start_and_endtimes_per_doc(schedule, key, start_time=0):
...
def calculate_start_and_endtimes_per_doc_non_det(schedule, key, start_time=0):
...
def swap_patients_by_doc_N1(schedule, key, patient_1, patient_2, Max_d,deterministic, start_time=0):
    temp_schedule = deepcopy(schedule)
    temp_schedule[key][[patient_1, patient_2]] = temp_schedule[key][[patient_2, patient_1]]
    if deterministic:
        calculate_start_and_endtimes_per_doc(schedule=temp_schedule, key=key, start_time=start_time)
    else:
        calculate_start_and_endtimes_per_doc_non_det(schedule=temp_schedule, key=key, start_time=start_time)
...
def insert_patient_by_doc_N2(schedule, key, insert_patient_id, in_front_of_patient_id, Max_d,deterministic, start_time=0):
    temp_schedule = deepcopy(schedule)
...
def swap_patients_between_docs_N3(schedule, patient_1, patient_2, Max_d, deterministic, start_time_1=0, start_time_2=0):
    ...
    if deterministic:
        calculate_start_and_endtimes_per_doc(schedule=temp_schedule, key=key1, start_time=start_time_1)
        calculate_start_and_endtimes_per_doc(schedule=temp_schedule, key=key2, start_time=start_time_2)
    else:
        calculate_start_and_endtimes_per_doc_non_det(schedule=temp_schedule, key=key1, start_time=start_time_1)
        calculate_start_and_endtimes_per_doc_non_det(schedule=temp_schedule, key=key2, start_time=start_time_2)
...
def local_search(schedule, start_time_array ,neighborhood, Max_d=120, deterministic=True):
    temp_schedule = deepcopy(schedule)
    overall_tardiness = 0
    if neighborhood == "N1":
        for key in schedule:
            if schedule[key].shape[0] > 1:
                start_time = start_time_array[key-1]
                temp_schedule, lowest_tardiness = best_swap_N1(temp_schedule, key, Max_d=Max_d, start_time=start_time, deterministic=deterministic)
                #overall_tardiness += lowest_tardiness
    if neighborhood == "N2":
        for key in schedule:
            if schedule[key].shape[0] > 1:
                start_time = start_time_array[key-1]
                temp_schedule, lowest_tardiness = best_insert_N2(temp_schedule, key, Max_d=Max_d, start_time=start_time, deterministic=deterministic)
                #overall_tardiness += lowest_tardiness
    if neighborhood == "N3":
        ...
        for key1, key2 in key_pairs:
            if schedule[key1].shape[0] > 0 and schedule[key2].shape[0] > 0:
                start_time_1 = start_time_array[key1-1]
                start_time_2 = start_time_array[key2-1]
                temp_schedule, lowest_tardiness = best_swap_N3(temp_schedule, key1, key2, start_time_1=start_time_1, start_time_2=start_time_2, deterministic=deterministic)
...
def best_swap_N1(schedule, key, deterministic, Max_d = 120, start_time=0):
    ...
    for i in range(num_patients):
        for j in range(i + 1, num_patients):
            ...
            temp_schedule = deepcopy(schedule)
            temp_schedule = swap_patients_by_doc_N1(temp_schedule, key, i, j, Max_d=Max_d, start_time=start_time, deterministic=deterministic)
...
def best_insert_N2(schedule, key, deterministic, Max_d = 120, start_time=0):
    # Anzahl der Patienten bei diesem Doktor
    num_patients = schedule[key].shape[0]
    ...
    for i in range(num_patients):
        for j in range(num_patients):
            if i != j:
                ...
                temp_schedule = insert_patient_by_doc_N2(schedule, key, patient_id_to_move, patient_id_target, Max_d=Max_d, start_time=start_time, deterministic=deterministic)
...
def best_swap_N3(schedule, key1, key2, deterministic, Max_d = 120, start_time_1=0, start_time_2=0):
    ...
    for i in range(num_patients_key1):
        for j in range(num_patients_key2):
            # Führe den Tausch durch
            swapped_schedule = swap_patients_between_docs_N3(schedule, [key1, i], [key2, j], Max_d=Max_d, start_time_1=start_time_1, start_time_2=start_time_2, deterministic=deterministic)
...
def random_shake(schedule, Max_d, start_time_array, deterministic, operations=['N1', 'N2', 'N3']):
    temp_schedule = deepcopy(schedule)
    ...
    if operation == 'N3' and len(keys)>1:
        ...
        if temp_schedule[key1].shape[0] > 0 and temp_schedule[key2].shape[0] > 0:
            #print(f"Shake N3")
            index1 = random.randint(0, temp_schedule[key1].shape[0] - 1)
            index2 = random.randint(0, temp_schedule[key2].shape[0] - 1)
            start_time_1 = start_time_array[key1-1]
            start_time_2 = start_time_array[key2-1]
            temp_schedule = swap_patients_between_docs_N3(temp_schedule, [key1, index1], [key2, index2],  Max_d=Max_d, start_time_1=start_time_1, start_time_2=start_time_2, deterministic=deterministic)
        ...
        if temp_schedule[key].shape[0] > 1:
            ...
            temp_schedule = insert_patient_by_doc_N2(temp_schedule, key, patient_id_to_move, patient_id_target,  Max_d=Max_d, start_time=start_time, deterministic=deterministic)
...
def VND(schedule, overall_tardiness, neighborhoods,start_time_array, deterministic, Max_d, vns_start_time, max_time):
    ...
    while improvement:
        ...
        for neighborhood in neighborhoods:
            ...
            if neighborhood == "N1":
                new_schedule, _ = local_search(schedule, neighborhood="N1", Max_d=Max_d, start_time_array=start_time_array, deterministic=deterministic)
            elif neighborhood == "N2":
                new_schedule, _ = local_search(schedule, neighborhood="N2", Max_d=Max_d, start_time_array=start_time_array, deterministic=deterministic)
            elif neighborhood == "N3":
                new_schedule, _ = local_search(schedule, neighborhood="N3", Max_d=Max_d, start_time_array=start_time_array, deterministic=deterministic)
            else:
                continue

            # Vergleiche die neue und aktuelle Lösung
            if _ < current_tardiness:
                current_schedule = deepcopy(new_schedule)
                current_tardiness = deepcopy(_)
                improvement = True
                break  # Beginne die Suche erneut bei der ersten Nachbarschaft
            
            completion_time = time.time() - vns_start_time
...
def general_vns(initial_schedule, initial_tardiness ,neighborhoods, start_time_array, Max_d=120, no_improvement_limit=1000, time_limit = 60, deterministic=True):
    ...
    while completion_time <= time_limit:
        ...
        shaken_schedule, shaken_tardiness = random_shake(current_schedule, Max_d=Max_d, start_time_array=start_time_array, deterministic=deterministic)
        # VND: Anwenden der lokalen Optimierung
        vnd_schedule, vnd_tardiness = VND(shaken_schedule, shaken_tardiness, neighborhoods=neighborhoods, Max_d=Max_d, start_time_array=start_time_array, deterministic=deterministic, vns_start_time=start_time, max_time=time_limit)
...
def simulated_annealing(schedule, total_tardiness, start_time_array, Imax=1000, cooling_rate=0.95, min_temperature=1.0, start_temperature=100, time_limit=600, Max_d=120, deterministic=True):
    ...
    while temperature > min_temperature:
        ...
        while I <= Imax:
            ...
            #while new_tardiness == current_tardiness:
                #shaken_schedule, new_tardiness = random_shake(current_schedule, Max_d=Max_d, start_time_array=start_time_array)
            shaken_schedule, new_tardiness = random_shake(current_schedule, Max_d=Max_d, start_time_array=start_time_array, deterministic=deterministic)
            ...
            if delta < 0:
                ...
                if current_tardiness < current_best_solution_tardiness:
                    ...
                    if current_best_solution_tardiness == 0:
                        return (current_best_solution, current_best_solution_tardiness, temperature)
            else:
                ...
                if random.random() < acceptance_probability:
                    ...
                    current_tardiness = deepcopy(new_tardiness)
...
def simulated_annealing_by_neighborhood(schedule, total_tardiness, start_time_array, Imax=1000, cooling_rate=0.95, min_temperature=1.0, start_temperature=100, time_limit=600, Max_d=120, deterministic=True, operations=['N3']):
    current_schedule = deepcopy(schedule) # Start solution (S) is the result from the opening heuristic
    current_tardiness = deepcopy(total_tardiness) # Calculate the makespan of the start solution
    # Input seed for reproducibility if necessary
    np.random.seed()
    ...
    while temperature > min_temperature:
        ...
        while I <= Imax:
            ...
            #while new_tardiness == current_tardiness:
                #shaken_schedule, new_tardiness = random_shake(current_schedule, Max_d=Max_d, start_time_array=start_time_array)
            shaken_schedule, new_tardiness = random_shake(current_schedule, Max_d=Max_d, start_time_array=start_time_array, deterministic=deterministic, operations=operations)
            ...
            if delta < 0:
                ...
                if current_tardiness < current_best_solution_tardiness:
                    print(f"Better Solution: {(time.time() - start_time)},{temperature}, {I} {current_best_solution_tardiness} to {current_tardiness}; accepted bad Solutions: {bad_count}")
                    current_best_solution = deepcopy(current_schedule) # Set S* (currently best solution) to S'
                    current_best_solution_tardiness = deepcopy(current_tardiness)
            else:
                # Accept solutions with a certain probability
...
def general_vns_cyclic(initial_schedule, initial_tardiness ,neighborhoods, start_time_array, Max_d=120, no_improvement_limit=1000, time_limit = 60, deterministic=True):
    current_schedule = deepcopy(initial_schedule)
    best_schedule = current_schedule
    best_schedule_tardiness = initial_tardiness
    no_improvement_count = 0
    ...
    while completion_time <= time_limit:
        ...
        # Shake: Erzeugen einer neuen Lösung durch eine zufällige Veränderung
        shaken_schedule, shaken_tardiness = random_shake(current_schedule, Max_d=Max_d, start_time_array=start_time_array, deterministic=deterministic, operations=[neighborhoods[i]])
        # VND: Anwenden der lokalen Optimierung
        vnd_schedule, vnd_tardiness = VND(shaken_schedule, shaken_tardiness, neighborhoods=neighborhoods, Max_d=Max_d, start_time_array=start_time_array, deterministic=deterministic)
        if i == 2:
            i = 0
        else:
            i+=1
